...
1. 打开**设置 → 服务商**，添加 OpenAI 兼容配置。
2. 填写基础 URL、模型 ID 和 API 密钥。远程主机必须使用 HTTPS；明文 HTTP 只允许显式选择这台 Mac 上的标准回环主机。
3. 检查 UtterInk 显示的规范化主机，然后使用**测试连接**。该操作会请求 `GET /models`；如果配置中包含凭据则会携带，但不会发送转写文本。
4. 创建或选择带有已保存润色指令的自定义输出模式。
